...
// Polyglot Zobrist random tables
// These are the official Polyglot random numbers used for hashing positions.
...
// Standard EPD format entry
struct EPDEntry {
    std::string fen;       // FEN string of the position
    uint32_t move;         // Best move (encoded)
    uint16_t weight;       // Move selection weight
    uint16_t games;        // Number of games with this position
    uint32_t whiteWins;    // White's win count
    uint32_t blackWins;    // Black's win count
    uint32_t draws;        // Draw count
    uint64_t polyglotKey;  // Polyglot position key (0 if not from Polyglot book)
};

// Polyglot book entry (binary format) - STRICTLY 16 bytes
#pragma pack(push, 1)
struct PolyglotEntry {
    uint64_t key;          // 8 bytes - Polyglot Zobrist Key
    uint16_t move;         // 2 bytes - Compressed move
    uint16_t weight;       // 2 bytes - Move weight/popularity
    uint32_t learn;        // 4 bytes - Default 0
};
#pragma pack(pop)

static_assert(sizeof(PolyglotEntry) == 16, "PolyglotEntry must be exactly 16 bytes");

// Polyglot book header (for files that have one - we tolerate both)
struct [[gnu::packed]] PolyglotHeader {
    char magic[8];         // "pglb" magic number
    uint32_t version;      // Version number (0)
    uint32_t reserved[3];  // Reserved, must be 0
    uint32_t tableCount;   // Number of hash tables
    uint32_t entrySize;    // Size of each entry in bytes (must be 20)
    uint32_t minMoves;     // Minimum number of moves per position
    uint32_t maxMoves;     // Maximum number of moves per position
    uint32_t padding;      // Padding to 64 bytes
};

// Opening Book Manager class
class BookManager {
public:
    BookManager();
    ~BookManager();

    // Load opening book from standard EPD file
    // Standard EPD format: FEN move1 weight1 move2 weight2 ...;
    // Lines starting with ';' are comments
    bool loadBook(const std::string& filePath);

    // Load standard EPD file (text format)
    bool loadEPDBook(const std::string& filePath);

    // Check if a position is in the book and get the best move
    // Returns std::nullopt if position not found or book not loaded
    std::optional<Move> getBookMove(const Position& pos, int depth = 0) const;

    // Get weighted random book move for current position
    // Uses weights to select moves proportionally
    std::optional<Move> getWeightedBookMove(const Position& pos) const;

    // Compute Polyglot hash for a position using official Polyglot Zobrist keys
    uint64_t generatePolyglotKey(const Position& pos) const;

    // Get book statistics
    int getBookSize() const { return static_cast<int>(entries_.size()); }
    
    bool isBookLoaded() const { return bookPath_ != ""; }

    // Set default book file path
    void setDefaultBookPath(const std::string& path) { defaultBookPath_ = path; }

    // Set whether to use default book or user-provided book
    void setUseDefaultBook(bool use) { useDefaultBook_ = use; }

    // Set book path (for GUI/user selection)
    void setBookPath(const std::string& path) { bookPath_ = path; }

    // Get current book path
    std::string getBookPath() const { return bookPath_; }

    // Clear the book
    void clearBook();

    // Get book format type
    enum class BookType {
        None,
        EPD,
        Polyglot
    };
    BookType getBookType() const { return bookType_; }

    // Get all moves for a given position from the Polyglot book
    // Returns a vector of (move, weight) pairs for all available moves
    struct BookMove {
        Move move;
        uint16_t weight;
    };
    std::vector<BookMove> getAllBookMoves(const Position& pos) const;

private:
    std::vector<EPDEntry> entries_;
    std::string defaultBookPath_;
    std::string bookPath_;
    bool useDefaultBook_;
    bool isPolyglot_ = false;
    BookType bookType_;

    // Parse standard EPD line and extract position info
    bool parseEPDLine(const std::string& line, EPDEntry& entry) const;

    // Parse algebraic move notation to encoded move
    uint32_t parseAlgebraicMove(const std::string& moveStr) const;

    // Convert FEN to Zobrist hash for book lookup
    uint64_t fenToZobrist(const std::string& fen) const;

    // Parse a single Polyglot entry from binary data
    bool parsePolyglotEntry(const unsigned char* data, PolyglotEntry& entry) const;

    // Encode move for Polyglot format
    ...
    // Convert Big-Endian uint64 to Native (Little-Endian)
    static uint64_t be64ToNative(uint64_t be) {
        return ((be >> 56) & 0x00000000000000FFULL) |
               ((be >> 40) & 0x000000000000FF00ULL) |
               ((be >> 24) & 0x0000000000FF0000ULL) |
               ((be >>  8) & 0x00000000FF000000ULL) |
               ((be <<  8) & 0x000000FF00000000ULL) |
               ((be << 24) & 0x0000FF0000000000ULL) |
               ((be << 40) & 0x00FF000000000000ULL) |
               ((be << 56) & 0xFF00000000000000ULL);
    }

    // Convert Big-Endian uint16 to Native (Little-Endian)
    static uint16_t be16ToNative(uint16_t be) {
        return static_cast<uint16_t>((be >> 8) | (be << 8));
    }

    // Convert Big-Endian uint32 to Native (Little-Endian)
    static uint32_t be32ToNative(uint32_t be) {
        return ((be >> 24) & 0x000000FFU) |
               ((be >>  8) & 0x0000FF00U) |
               ((be <<  8) & 0x00FF0000U) |
               ((be << 24) & 0xFF000000U);
    }

    // Binary search for a Polyglot key in the book file
    // Returns the index of the FIRST entry with the matching key, or -1 if not found
    int64_t binarySearchKey(std::ifstream& file, uint64_t targetKey) const;

    // Unpack a Polyglot move into from/to squares and promotion type
    struct UnpackedMove {
        uint8_t fromSquare;
        uint8_t toSquare;
        uint8_t promotion; // 0=None, 1=Knight, 2=Bishop, 3=Rook, 4=Queen
    };
    UnpackedMove unpackPolyglotMove(uint16_t pgMove) const;
};
