...
const Move iterativeSearch(Position& p, Search& search, short const& depth)
{
	...
	if (moveList.size() == 0) {
		...
		for (short ply = 1; ply <= depth; ply++) {
			...
			if (search.bestMove) {
				...
				std::cout << "*depth:" << ply << " nodes:" << search.nodes << " ms:" << (unsigned int)(depthTime.count()) <<
					" total_ms:" << totalTime << " nps:" << (unsigned int)(search.nodes / (depthTime.count() / 1000))
					<< " TT_hits:" << search.ttHits << " TT_useful:" << search.ttUseful << "\n";

				std::cout << "\t move:" << displayMove(search.pos, search.bestMove) << " score:";
				
				if (!(search.bestScore == (MATE + search.height) && !(search.bestScore == -MATE - search.height))) {
  ...
  const short newPVS(Position& p, Search& search, short const& depth, short alpha, short beta, Move* pv)
  {
      ...
      const TTEntry* entryPtr = TT::Lookup(static_cast<uint32_t>(p.getZobrist()), static_cast<uint8_t>(depth));
      if (entryPtr) {
          search.ttHits++;
          search.ttUseful++;
      ...
      TTEntry entry;
      entry.key = static_cast<uint32_t>(p.getZobrist());
      entry.depth = static_cast<uint8_t>(depth);
      entry.score = bestScore;
      entry.move = bestMove;
      entry.age = static_cast<uint8_t>(p.getMoveNumber());

      if (bestScore <= alphaOrig)
          entry.nodeType = UPPER_BOUND; // Fall-low
      else if (bestScore >= beta)
          entry.nodeType = LOWER_BOUND; // Beta-cut
      else
          entry.nodeType = EXACT;       // PV-node
